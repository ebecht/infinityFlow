...
This vignette will cove:
1. Package installation
1. Setting up your input data
1. Specifying the Backbone and Infinity antibodies
1. Running the Infinity Flow computational pipeline
...
The `steady_state_lung_annotation` data.frame contains one line per FCS file, with `rownames(steady_state_lung_annotation) == sampleNames(steady_state_lung)`. The first column specifies the antibodies targeted by the Infinity antibody in each FCS file, and the second column specifies its isotype (species and constant region of the antibody).
...
Now that we have our input data, we need to specify which antibodies are part of the Backbone, and which one is the Infinity antibody. We provide an interactive function to specify this directly in R. This function can be run once, its output save for future use by downstream function in the ***infinityFlow*** package. The `select_backbone_and_exploratory_markers` function will parse an FCS file in the input directory, and for each acquisition channel, ask the user whether it should be used as a predictor (Backbone), exploratory target (Infinity antibodies), or omitted (e.g. Time or Event ID columns...). 
...
All the pipeline is package into a single function, `infinity_flow()`.

Here is a description of the basic arguments it requires:

1. path_to_fcs: path a the folder with input FCS files
...
Other arguments are not necessary, but it is worth considering the number of input cells and the number of output cells. This will notably be important if you are using a computer with limited RAM. For the example data it does not matter as we only have access to 2,000 cells per well, but if you run the pipeline on your own data I suggest you start by low values, and ramp up once everything is setup.
...
There is also an argument to store temporary files, which can be useful to further analyze the data in R:
...
```{r}
print(backbone_selection_file)
print(read.csv(backbone_selection_file, stringsAsFactors = FALSE))
```

```{r, pipeline execution, eval = TRUE, cache = TRUE}
...
1. Sets of FCS files with imputed data
2. PDF of a UMAP embedding of the Backbone data, color-coded by imputed expression of the Infinity antibody
...
fcs_raw <- file.path(path_to_output, "output", "FCS", "split")
head(list.files(fcs_raw)) ## Raw output FCS files
fcs_bgc <- file.path(path_to_output, "output", "FCS_background_corrected", "split") ## Background-corrected output FCS files
head(list.files(fcs_bgc)) ## Background-corrected output FCS files
```

Finally, the pipeline produces two PDF files, with a UMAP embedding of the backbone data color-coded by the imputed data. This is a very informative output and a good way to start analyzing your data. These files are present in the `path_to_output` directory. The example dataset is very small but here is the result for illustration purposes:
...
Thank you for following this vignette, I hope you made it through the end without too much headache and that it was informative. Feel free to raise an issue on infinityFlow's \url[github]{https://github.com/ebecht/infinityFlow/issues} if you run into unsolvable issues.
